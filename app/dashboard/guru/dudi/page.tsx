...
import { getDudiGuruData } from "./action"
...
export default function DudiGuruPage() {
  ...
  useEffect(() => {
    async function initData() {
      setLoading(true)
      const { data } = await getDudiGuruData()
      setDudiList(data)
      setLoading(false)
    }
    initData()
  ...
  const totalSiswaAktif = dudiList.reduce((total, dudi) => total + (dudi.siswa_count || 0), 0)
  const rataRata = dudiList.length > 0 ? Math.round(totalSiswaAktif / dudiList.length) : 0
        ...
        <div className="p-6 border-b border-slate-50 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <div className="relative">
            ...
            <Input 
              ...
              className="pl-9 w-full md:w-[350px] border-slate-200 rounded-xl focus-visible:ring-cyan-500 text-sm" 
            />
          </div>
        </div>
      ...
      <td className="px-6 py-5">
        <div className="flex items-center gap-3">
          ...
          <div className="max-w-[200px]">
            <p className="text-sm font-bold text-slate-800 leading-tight truncate">{company}</p>
            <p className="text-[10px] text-slate-400 mt-1 truncate">{address || "Alamat belum tersedia"}</p>
          </div>
        </div>
      </td>
      <td className="px-6 py-5">
